...
type ScanResult =
  | { status: 'valid'; ticketId: string; tierName: string; buyerName: string; eventName: string }
...
export default function PublicScannerPage() {
  ...
  if (!scannerToken) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-6 text-center">
        ...
        <h1 className="text-xl font-black text-white mb-2">Invalid Scanner Link</h1>
        <p className="text-white/40 text-sm">This link is missing the scanner token. Ask the organizer to share the correct link.</p>
      </div>
    )
  }
  ...
  async function checkIn(token: string) {
    ...
    try {
      const res = await fetch('/api/organizer/checkin', {
        ...
        body: JSON.stringify({ qrToken: token, scannerToken }),
      })
  ...
  const resultTitle =
    ...
    result?.status === 'already_used' ? 'Already checked in' :
    result?.status === 'invalid' ? 'Invalid ticket' :
    result?.status === 'error' && result.message === 'Unauthorized' ? 'Scanner link expired' :
    'Error'
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center text-sm"
            style={{ background: 'rgba(200,255,0,0.12)' }}>🎟</div>
          <span className="text-white/40 text-xs font-bold uppercase tracking-wider">Door Scanner</span>
        </div>
        <h1 className="text-2xl font-black text-white">Scan Tickets</h1>
        <p className="text-white/40 mt-1 text-sm">Point camera at attendee QR code to check them in.</p>
      </div>
      ...
      {result && (
        <div className="glass rounded-2xl p-5 mb-4" style={{ borderColor: `${resultColor}40`, borderWidth: 1 }}>
          ...
          {result.status === 'invalid' && (
            <p className="text-center text-red-400/70 text-sm">QR code not recognized</p>
          )}
          {result.status === 'error' && result.message === 'Unauthorized' && (
            <p className="text-center text-red-400/70 text-sm">This scanner link may have been regenerated. Ask the organizer for a new link.</p>
          )}
          {result.status === 'error' && result.message !== 'Unauthorized' && (
            <p className="text-center text-red-400/70 text-sm">{result.message}</p>
          )}
        </div>
      )}
