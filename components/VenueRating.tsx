...
const CATEGORIES = [
  { key: 'sound',       label: 'Sound System',   emoji: '🔊', desc: 'Clarity, bass, coverage' },
  { key: 'dance_floor', label: 'Dance Floor',     emoji: '💃', desc: 'Size, layout, energy' },
  { key: 'bar',         label: 'Bar & Drinks',    emoji: '🍺', desc: 'Speed, prices, selection' },
  { key: 'bathrooms',   label: 'Bathrooms',       emoji: '🚿', desc: 'Cleanliness & wait times' },
  { key: 'location',    label: 'Location',        emoji: '📍', desc: 'Access, parking, transit' },
...
function StarRow({ value, onChange, disabled }: { value: number; onChange: (v: number) => void; disabled: boolean }) {
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(star => (
        <button key={star} disabled={disabled}
...
export default function VenueRating({ venueSlug, venueName }: { venueSlug: string; venueName: string }) {
  const { user } = useAuth()
  ...
  const storageKey = `venue-votes-${venueSlug}`

  useEffect(() => {
    fetch(`/api/venues/${venueSlug}/votes`)
      .then(r => r.json())
  ...
  }, [venueSlug, storageKey])

  const hasVotes = Object.keys(serverVotes).length > 0
  const myVoteCount = Object.keys(myVotes).length
  const overallAvg = hasVotes
  ...
  async function handleVote(category: string, rating: number) {
    ...
    if (current) {
      ...
      const newCount = prev ? current.count : current.count + 1
      const newAvg = prev
        ? (current.avg * current.count - prev + rating) / newCount
        : (current.avg * current.count + rating) / newCount
      setServerVotes(p => ({ ...p, [category]: { avg: Math.round(newAvg * 10) / 10, count: newCount } }))
    } else {
      setServerVotes(p => ({ ...p, [category]: { avg: rating, count: 1 } }))
    }

    try {
      const res = await fetch(`/api/venues/${venueSlug}/votes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ category, rating, userId: user.id, venueName }),
      })
      if (res.ok) {
          ...
          <div className="flex flex-col gap-2">
            {CATEGORIES.filter(c => serverVotes[c.key]).map(cat => (
              <div key={cat.key} className="flex items-center gap-2">
                ...
                <span className="text-white/50 text-xs w-24 flex-shrink-0">{cat.label}</span>
                <RatingBar avg={serverVotes[cat.key].avg} count={serverVotes[cat.key].count} />
              </div>
            ))}
          </div>
          <p className="text-white/25 text-xs mt-3 text-center">Tap to {user ? 'rate' : 'see details'}</p>
        </button>
      )}

      {!expanded && !hasVotes && (
        <button onClick={() => setExpanded(true)} className="w-full glass p-6 rounded-2xl text-center">
          <p className="text-3xl mb-2">⭐</p>
          <p className="text-white/60 font-bold text-sm">No ratings yet</p>
          <p className="text-white/30 text-xs mt-1">
            {user ? `Be the first to rate ${venueName}` : 'Sign in to rate this venue'}
          </p>
        </button>
      )}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-2.5 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {sv && <div className="mt-1.5"><RatingBar avg={sv.avg} count={sv.count} /></div>}
                    </div>
                  </div>
                  <div className="flex-shrink-0">
                    {user ? (
                      <div className="flex items-center gap-1">
                        ...
                        <StarRow value={myRating} onChange={v => handleVote(cat.key, v)} disabled={saving === cat.key} />
                      </div>
                    ) : (
            ...
            {user ? (
              <>
                <p className="text-white/25 text-xs text-center">
                  {myVoteCount === 0 ? 'Rate based on your experience at this venue'
                    : myVoteCount < CATEGORIES.length ? `${myVoteCount}/${CATEGORIES.length} rated — keep going`
                    : '✓ All categories rated — thanks!'}
                </p>
